...
public class PackProgressDialog extends AbstractProgressDialog {
	...
	public void run() {
		...
		switch (type) {
			case ZIP -> createZIP(currentPath, fileName + ".zip", files);
			case JAR -> createJAR(currentPath, fileName + ".jar", files);
			case GZIP -> createGZIP(currentPath, fileName + ".gzip", files);
		}
	}
	...
	private void createZIP(String currentPath, String fullName, List<File> files) {
		try (FileOutputStream fos = new FileOutputStream(fullName);
			...
			for (int i = 0; i < files.size(); i++) {
				...
				try (var fis = new FileInputStream(myProcess.currentFile)) {
